  ...
  static TypeId tid =
      TypeId("ns3::RdmaHw")
          ...
          .AddAttribute("MinRate", "Minimum rate of a throttled flow",
                        ...
                        MakeDataRateChecker())
          .AddAttribute("Mtu", "Mtu.", UintegerValue(1000),
          ...
          .AddAttribute("NACK_Generation_Interval",
                        "The NACK Generation interval", DoubleValue(500.0),
          ...
          .AddAttribute("EwmaGain",
                        "Control gain parameter which determines the level of "
                        "rate decrease",
                        DoubleValue(1.0 / 16), MakeDoubleAccessor(&RdmaHw::m_g),
                        MakeDoubleChecker<double>())
          .AddAttribute("RateOnFirstCnp", "the fraction of rate on first CNP",
                        DoubleValue(1.0),
                        MakeDoubleAccessor(&RdmaHw::m_rateOnFirstCNP),
                        MakeDoubleChecker<double>())
          .AddAttribute("ClampTargetRate", "Clamp target rate.",
                        BooleanValue(false),
                        MakeBooleanAccessor(&RdmaHw::m_EcnClampTgtRate),
                        MakeBooleanChecker())
          .AddAttribute(
              "RPTimer", "The rate increase timer at RP in microseconds",
              DoubleValue(1500.0), MakeDoubleAccessor(&RdmaHw::m_rpgTimeReset),
              MakeDoubleChecker<double>())
          .AddAttribute("RateDecreaseInterval",
                        "The interval of rate decrease check", DoubleValue(4.0),
                        MakeDoubleAccessor(&RdmaHw::m_rateDecreaseInterval),
                        MakeDoubleChecker<double>())
          .AddAttribute("FastRecoveryTimes", "The rate increase timer at RP",
                        UintegerValue(5),
                        MakeUintegerAccessor(&RdmaHw::m_rpgThreshold),
                        MakeUintegerChecker<uint32_t>())
          .AddAttribute("AlphaResumInterval", "The interval of resuming alpha",
                        DoubleValue(55.0),
          ...
          .AddAttribute("RateAI", "Rate increment unit in AI period",
                        ...
                        MakeDataRateChecker())
          .AddAttribute(
              "RateHAI", "Rate increment unit in hyperactive AI period",
              DataRateValue(DataRate("50Mb/s")),
              MakeDataRateAccessor(&RdmaHw::m_rhai), MakeDataRateChecker())
          .AddAttribute(
              "VarWin", "Use variable window size or not", BooleanValue(false),
              MakeBooleanAccessor(&RdmaHw::m_var_win), MakeBooleanChecker())
          .AddAttribute("FastReact", "Fast React to congestion feedback",
                        BooleanValue(true),
                        MakeBooleanAccessor(&RdmaHw::m_fast_react),
                        MakeBooleanChecker())
          .AddAttribute(
              "MiThresh", "Threshold of number of consecutive AI before MI",
              UintegerValue(5), MakeUintegerAccessor(&RdmaHw::m_miThresh),
              MakeUintegerChecker<uint32_t>())
          .AddAttribute("TargetUtil",
                        "The Target Utilization of the bottleneck bandwidth, "
                        "by default 95%",
                        DoubleValue(0.95),
          ...
          .AddAttribute("UtilHigh",
                        ...
                        MakeDoubleChecker<double>())
          .AddAttribute(
              "RateBound", "Bound packet sending by rate, for test only",
              BooleanValue(true), MakeBooleanAccessor(&RdmaHw::m_rateBound),
              MakeBooleanChecker())
          .AddAttribute("MultiRate", "Maintain multiple rates in HPCC",
                        BooleanValue(true),
                        MakeBooleanAccessor(&RdmaHw::m_multipleRate),
                        MakeBooleanChecker())
          .AddAttribute("SampleFeedback", "Whether sample feedback or not",
                        BooleanValue(false),
          ...
          .AddAttribute("TimelyBeta", "Beta of TIMELY", DoubleValue(0.8),
                        ...
                        MakeDoubleChecker<double>())
          .AddAttribute("TimelyTLow", "TLow of TIMELY (ns)",
                        UintegerValue(50000),
                        MakeUintegerAccessor(&RdmaHw::m_tmly_TLow),
                        MakeUintegerChecker<uint64_t>())
          .AddAttribute("TimelyTHigh", "THigh of TIMELY (ns)",
                        UintegerValue(500000),
                        MakeUintegerAccessor(&RdmaHw::m_tmly_THigh),
                        MakeUintegerChecker<uint64_t>())
          .AddAttribute("TimelyMinRtt", "MinRtt of TIMELY (ns)",
                        UintegerValue(20000),
                        MakeUintegerAccessor(&RdmaHw::m_tmly_minRtt),
                        MakeUintegerChecker<uint64_t>())
          .AddAttribute(
              "DctcpRateAI", "DCTCP's Rate increment unit in AI period",
...
void RdmaHw::AddQueuePair(uint32_t src, uint32_t dest, uint64_t tag,
                          uint64_t size, uint16_t pg, Ipv4Address sip,
...
Ptr<RdmaRxQueuePair> RdmaHw::GetRxQp(uint32_t sip, uint32_t dip, uint16_t sport,
                                     uint16_t dport, uint16_t pg, bool create) {
...
uint32_t RdmaHw::GetNicIdxOfRxQp(Ptr<RdmaRxQueuePair> q) {
  ...
  if (v.size() > 0) {
    ...
    std::cout << "ROUTE NOT FOUND for IP: ";
    Ipv4Address(q->dip).Print(std::cout);
    std::cout << " on node " << m_node->GetId() << std::endl;
    std::fflush(stdout);
    NS_ASSERT_MSG(false, "We assume at least one NIC is alive");
    return 0;
  }
...
void RdmaHw::EnablePause() { enable_pcie_pause = true; }
...
void RdmaHw::PCIePause(uint32_t nic_idx, uint32_t qIndex) {
  ...
  m_nic[nic_idx].dev->RdmaEnqueueHighPrioQ(p);
  m_nic[nic_idx].dev->TriggerTransmit();
  std::cout << "NIC pause " << m_node->GetId() << " at "
            << Simulator::Now().GetNanoSeconds() << " paused " << m_paused_times
            << std::endl;
  Simulator::Schedule(MicroSeconds(10), &RdmaHw::PCIeResume, this, nic_idx,
                      qIndex);
}
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch) {
  ...
  if (enable_pcie_pause) { //&& ((m_node->GetId())%16 == 0 ||
                           //(m_node->GetId())%16 == 8)) { //&&
  ...
  if (x != 1 && x != 2) {
    ...
    // 		std::cout << " " << ch.udp.dport << " " << ch.udp.seq << " " <<
    // ch.udp.pg << " " << p->GetSize() << " " << payload_size;
...
int RdmaHw::Receive(Ptr<Packet> p, CustomHeader &ch) {
  uint32_t my_id = m_node->GetId();
  uint32_t dst_id = (ch.dip >> 8) & 0xffff;

  if (dst_id != my_id) {
    auto &v = m_rtTable[ch.dip];
    if (v.size() > 0) {
      uint32_t nic_id = v[0];
      uint32_t pg = 3;
      if (ch.l3Prot == 0x11)
        ...
        pg = ch.ack.pg;
      m_nic[nic_id].dev->SwitchSend(pg, p, ch);
      return 0;
    } else {
      std::cout << "ROUTE NOT FOUND for IP: ";
      Ipv4Address(ch.dip).Print(std::cout);
      std::cout << " on node " << my_id << " (packet dropped)" << std::endl;
      std::fflush(stdout);
      return 0;
    }
  }
...
int RdmaHw::ReceiverCheckSeq(uint32_t seq, Ptr<RdmaRxQueuePair> q,
                             uint32_t size) {
...
void RdmaHw::PktSent(Ptr<RdmaQueuePair> qp, Ptr<Packet> pkt,
                     Time interframeGap) {
  qp->lastPktSize = pkt->GetSize();
  UpdateNextAvail(qp, interframeGap, pkt->GetSize());
}

void RdmaHw::UpdateNextAvail(Ptr<RdmaQueuePair> qp, Time interframeGap,
                             uint32_t pkt_size) {
  Time sendingTime;
  if (m_rateBound)
...
void RdmaHw::cnp_received_mlx(Ptr<RdmaQueuePair> q) {
  ...
  if (q->mlx.m_first_cnp) {
    ...
    ScheduleDecreaseRateMlx(
        q, 1); // add 1 ns to make sure rate decrease is after alpha update
...
void RdmaHw::CheckRateDecreaseMlx(Ptr<RdmaQueuePair> q) {
  ...
  if (q->mlx.m_decrease_cnp_arrived) {
    ...
    printf("%lu rate dec: %08x %08x %u %u (%0.3lf %.3lf)->",
           Simulator::Now().GetTimeStep(), q->sip.Get(), q->dip.Get(), q->sport,
           q->dport, q->mlx.m_targetRate.GetBitRate() * 1e-9,
...
void RdmaHw::UpdateRateHp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p,
                          CustomHeader &ch, bool fast_react) {
    ...
    if (ih.nhop <= IntHeader::maxHop) {
      ...
      for (uint32_t i = 0; i < ih.nhop; i++) {
        ...
        if (print)
          printf(" %u(%u) %lu(%lu) %lu(%lu)", ih.hop[i].GetQlen(),
                 qp->hp.hop[i].GetQlen(), ih.hop[i].GetBytes(),
                 qp->hp.hop[i].GetBytes(), ih.hop[i].GetTime(),
                 qp->hp.hop[i].GetTime());
#endif
      ...
      if (!m_multipleRate) {
        ...
        if (updated_any) {
          if (dt > qp->m_baseRtt)
            dt = qp->m_baseRtt;
          qp->hp.u = (qp->hp.u * (qp->m_baseRtt - dt) + U * dt) /
                     double(qp->m_baseRtt);
...
#if PRINT_LOG
          if (print)
            printf(" u=%.6lf U=%.3lf dt=%u max_c=%.3lf", qp->hp.u, U, dt,
                   max_c);
#endif
#if PRINT_LOG
          if (print)
...
void RdmaHw::FastReactHp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p,
                         CustomHeader &ch) {
  if (m_fast_react)
 ...
 * TIMELY
 *********************/
void RdmaHw::HandleAckTimely(Ptr<RdmaQueuePair> qp, Ptr<Packet> p,
                             CustomHeader &ch) {
  uint32_t ack_seq = ch.ack.seq;
  // update rate
  if (ack_seq >
...
void RdmaHw::UpdateRateTimely(Ptr<RdmaQueuePair> qp, Ptr<Packet> p,
                              CustomHeader &ch, bool us) {
  uint32_t next_seq = qp->snd_nxt;
  ...
  if (qp->tmly.m_lastUpdateSeq != 0) { // not first RTT
    ...
    if (print)
      printf("%lu node:%u rtt:%lu rttDiff:%.0lf gradient:%.3lf rate:%.3lf",
             Simulator::Now().GetTimeStep(), m_node->GetId(), rtt, rtt_diff,
             gradient, qp->tmly.m_curRate.GetBitRate() * 1e-9);
#endif
    if (rtt < m_tmly_TLow) {
      inc = true;
...
void RdmaHw::FastReactTimely(Ptr<RdmaQueuePair> qp, Ptr<Packet> p,
                             CustomHeader &ch) {}

/**********************
 * DCTCP
 *********************/
void RdmaHw::HandleAckDctcp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p,
                            CustomHeader &ch) {
  uint32_t ack_seq = ch.ack.seq;
  uint8_t cnp = (ch.ack.flags >> qbbHeader::FLAG_CNP) & 1;
  ...
  if (ack_seq >
    ...
    printf("%lu %s %08x %08x %u %u [%u,%u,%u] %.3lf->",
           Simulator::Now().GetTimeStep(), "alpha", qp->sip.Get(),
           qp->dip.Get(), qp->sport, qp->dport, qp->dctcp.m_lastUpdateSeq,
    ...
    if (qp->dctcp.m_lastUpdateSeq == 0) { // first RTT
      ...
      double frac = std::min(1.0, double(qp->dctcp.m_ecnCnt) /
                                      qp->dctcp.m_batchSizeOfAlpha);
...
#if PRINT_LOG
    printf("%lu %s %08x %08x %u %u %.3lf->", Simulator::Now().GetTimeStep(),
           "rate", qp->sip.Get(), qp->dip.Get(), qp->sport, qp->dport,
...
void RdmaHw::HandleAckHpPint(Ptr<RdmaQueuePair> qp, Ptr<Packet> p,
                             CustomHeader &ch) {
  uint32_t ack_seq = ch.ack.seq;
...
void RdmaHw::UpdateRateHpPint(Ptr<RdmaQueuePair> qp, Ptr<Packet> p,
                              CustomHeader &ch, bool fast_react) {
  uint32_t next_seq = qp->snd_nxt;
