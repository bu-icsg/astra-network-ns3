...
TypeId RdmaEgressQueue::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::RdmaEgressQueue")
          .SetParent<Object>()
          .AddTraceSource(
              "RdmaEnqueue", "Enqueue a packet in the RdmaEgressQueue.",
              MakeTraceSourceAccessor(&RdmaEgressQueue::m_traceRdmaEnqueue),
              "ns3::Packet::TracedCallback")
          .AddTraceSource(
              "RdmaDequeue", "Dequeue a packet in the RdmaEgressQueue.",
  ...
  static TypeId tid =
      TypeId("ns3::QbbNetDevice")
          ...
          .AddAttribute("QcnEnabled", "Enable the generation of PAUSE packet.",
                        ...
                        MakeBooleanChecker())
          .AddAttribute("DynamicThreshold", "Enable dynamic threshold.",
                        BooleanValue(false),
          ...
          .AddTraceSource(
              "QbbEnqueue", "Enqueue a packet in the QbbNetDevice.",
              MakeTraceSourceAccessor(&QbbNetDevice::m_traceEnqueue),
              "ns3::Packet::TraceCallback")
          .AddTraceSource(
              "QbbDequeue", "Dequeue a packet in the QbbNetDevice.",
          ...
          .AddTraceSource("QbbDrop", "Drop a packet in the QbbNetDevice.",
                          ...
                          "ns3::Packet::TraceCallback")
          .AddTraceSource(
              "RdmaQpDequeue", "A qp dequeue a packet.",
...
void QbbNetDevice::TransmitComplete(void) {
  ...
  NS_ASSERT_MSG(m_currentPkt,
                "QbbNetDevice::TransmitComplete(): m_currentPkt zero");
...
void QbbNetDevice::DequeueAndTransmit(void) {
  ...
  if (m_node->GetNodeType() == 0) {
    if (m_queue && m_queue->GetNPackets() > 0) {
    ...
    if (p) {
      ...
      NS_LOG_INFO("PAUSE prohibits send at node " << m_node->GetId());
      if (m_node->GetNodeType() == 0 &&
          m_qcnEnabled) { // nothing to send, possibly due to qcn flow control,
                          // if so reschedule sending
        Time t = Simulator::GetMaximumSimulationTime();
        for (uint32_t i = 0; i < m_rdmaEQ->GetFlowCount(); i++) {
          Ptr<RdmaQueuePair> qp = m_rdmaEQ->GetQp(i);
          t = Min(qp->m_nextAvail, t);
        ...
        if (m_nextSend.IsExpired() &&
            t < Simulator::GetMaximumSimulationTime() && t > Simulator::Now()) {
          m_nextSend = Simulator::Schedule(
              t - Simulator::Now(), &QbbNetDevice::DequeueAndTransmit, this);
        }
      }
...
void QbbNetDevice::Resume(unsigned qIndex) {
  ...
  NS_LOG_INFO("Node " << m_node->GetId() << " dev " << m_ifIndex << " queue "
                      << qIndex << " resumed at "
...
bool QbbNetDevice::Send(Ptr<Packet> packet, const Address &dest,
...
void QbbNetDevice::SendPfc(uint32_t qIndex, uint32_t type) {
  ...
  Ptr<UniformRandomVariable> rng_generator =
...
Ptr<Packet> QbbNetDevice::NICSendPfc(uint32_t qIndex, uint32_t type) {
  ...
  PauseHeader pauseh((type == 0 ? m_pausetime : 0), m_queue->GetNBytes(qIndex),
                     qIndex);
  // std::cout << "m_pausetime " << pauseh.GetTime() << " " <<
  // pauseh.GetQIndex() <<std::endl;
  p->AddHeader(pauseh);
  Ipv4Header ipv4h; // Prepare IPv4 header
  ipv4h.SetProtocol(0xFE);
